...
[![Maven Central](https://img.shields.io/maven-central/v/io.mebius/mebius-android-sdk.svg)](https://central.sonatype.com/artifact/io.mebius/mebius-android-sdk)
...
Two ways in. Maven Central is the one to prefer — it needs no extra repository
line, because `mavenCentral()` is already in almost every project. JitPack stays
supported, and is the only way to resolve a tag Central does not have yet.
...
No `settings.gradle.kts` change: `mavenCentral()` is already there.

Central lags the newest tag when a release has not been pushed to it yet — check
[the versions on Central](https://central.sonatype.com/artifact/io.mebius/mebius-android-sdk/versions)
against the git tags, and use JitPack below if you need one that is missing.
...
```kotlin
dependencies {
    // <ref> = a release tag (v0.2.3), a commit hash, or `main-SNAPSHOT`
    implementation("com.github.russimobiledroidx:mebius-android-sdk:v0.2.3")
}
```
...
Releasing to Central is covered in [PUBLISHING.md](PUBLISHING.md).
